...
function ShiftsModule() {
  ...
  const closeShift = async () => {
    ...
    if(closedShift) {
      setReportModal(closedShift)
      // Auto-print to Bluetooth printer if connected
      if(BB_PRINTER.connected) {
        try {
          const t = calcShiftTotals(closedShift)
          const cashV = parseFloat(closedShift.cash_variance||0)
          const s = getReceiptSettings()
          const W = getPaperChars(s.paperWidth||"58")
          const ctr = (str) => { const p=Math.floor((W-String(str).length)/2); return " ".repeat(Math.max(0,p))+str }
          ...
          text(closedShift.date + "\n")
          add(ESC_ALIGN_LEFT)
          text(sep("=") + "\n")
          text(two("Opened by:", staffList.find(x=>x.id===closedShift.opened_by)?.name||"—") + "\n")
          text(two("Opened at:", new Date(closedShift.opened_at).toLocaleTimeString("en-ZA",{hour:"2-digit",minute:"2-digit"})) + "\n")
          text(two("Closed by:", staffList.find(x=>x.id===closedShift.closed_by)?.name||"—") + "\n")
          text(two("Closed at:", new Date(closedShift.closed_at||new Date()).toLocaleTimeString("en-ZA",{hour:"2-digit",minute:"2-digit"})) + "\n")
          text(two("Opening float:", `R${parseFloat(closedShift.open_float||0).toFixed(2)}`) + "\n")
          text(sep("-") + "\n")
          add(ESC_BOLD_ON)
          text("SALES SUMMARY\n")
          add(ESC_BOLD_OFF)
          text(two("Total orders:", t.count) + "\n")
          text(two("Cash sales:", `R${t.cash.toFixed(2)}`) + "\n")
          text(two("Card sales:", `R${t.card.toFixed(2)}`) + "\n")
          text(two("EFT sales:", `R${t.eft.toFixed(2)}`) + "\n")
          if(t.tips>0) text(two("Tips:", `R${t.tips.toFixed(2)}`) + "\n")
          text(sep("=") + "\n")
          add(ESC_BOLD_ON)
          add(ESC_DBLHEIGHT_ON)
          text(two("TOTAL REVENUE:", `R${t.total.toFixed(2)}`) + "\n")
          add(ESC_BOLD_OFF)
          text(sep("-") + "\n")
          add(ESC_BOLD_ON)
          text("CASH RECONCILIATION\n")
          add(ESC_BOLD_OFF)
          text(two("System cash:", `R${t.cash.toFixed(2)}`) + "\n")
          text(two("Float:", `R${parseFloat(closedShift.open_float||0).toFixed(2)}`) + "\n")
          text(two("Expected in drawer:", `R${(t.cash+parseFloat(closedShift.open_float||0)).toFixed(2)}`) + "\n")
          text(two("Cash counted:", `R${parseFloat(closedShift.cash_counted||0).toFixed(2)}`) + "\n")
          add(ESC_BOLD_ON)
          text(two("Variance:", `${cashV>=0?"+":""}R${cashV.toFixed(2)}`) + "\n")
          add(ESC_BOLD_OFF)
          if(closedShift.notes) {
            text(sep("-") + "\n")
            text("Notes: " + closedShift.notes + "\n")
          }
          text(sep("=") + "\n")
          add(ESC_ALIGN_CENTER)
          text("Report generated by BrewBase\n")
          text(new Date().toLocaleString("en-ZA") + "\n")
          add(FEED_LINES(4))
          if(s.cutAfter!==false) add(GS_CUT)
          await BB_PRINTER.print(bytes)
        } catch(e) {
          console.warn("Shift report print failed:", e)
        }
      }
    }
  }
